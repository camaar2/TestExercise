  ...
  return __asyncGenerator(this, null, function* () {
    try {
      for (var iter = __forAwait(iterable), more, temp, error; more = !(temp = yield new __await(iter.next())).done; more = false) {
        ...
        yield* __yieldStar(streamChunk(ArrayBuffer.isView(chunk) ? chunk : yield new __await(encoder.encode(String(chunk))), chunkSize));
      }
    } catch (temp) {
      error = [temp];
    } finally {
...
var getBodyLength = (body) => __async(void 0, null, function* () {
  ...
  if (utils_default.isString(body)) {
    return (yield new TextEncoder().encode(body)).byteLength;
  }
});
